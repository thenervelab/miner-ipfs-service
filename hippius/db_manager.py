...
async def update_cid_status(cid: str, status: str, retry_count: int = None):
    ...
    try:
        async with aiosqlite.connect(DATABASE_NAME) as db:
            ...
            await db.commit()
            logging.debug(
                f"CID {cid} status updated to {status}"
                + (f", retry count {retry_count}" if retry_count is not None else "")
            )
    except aiosqlite.Error as e:
    ...
    try:
        async with aiosqlite.connect(DATABASE_NAME) as db:
            async with db.execute(
                "SELECT cid, retry_count FROM pinned_cids WHERE status = ?", (status,)
            ) as cursor:
...
async def get_unpinnable_cids_to_report():
    ...
    try:
        async with aiosqlite.connect(DATABASE_NAME) as db:
            async with db.execute(
                "SELECT cid, reason FROM unpinnable_cids WHERE reported = FALSE"
            ) as cursor:
                return await cursor.fetchall()
    except aiosqlite.Error as e:
...
async def set_active_miner_profile(profile_cid: str | None):
    ...
    try:
        async with aiosqlite.connect(DATABASE_NAME) as db:
            ...
            await db.execute(
                "UPDATE miner_profile SET is_active = FALSE, pinned_locally = FALSE WHERE is_active = TRUE"
            )
            ...
            if profile_cid:
                ...
                logging.info(
                    "No active miner profile set (or current profile deactivated)."
                )
...
async def get_active_miner_profile() -> tuple | None:
    ...
    try:
        async with aiosqlite.connect(DATABASE_NAME) as db:
            async with db.execute(
                "SELECT profile_cid, pinned_locally FROM miner_profile WHERE is_active = TRUE"
            ) as cursor:
...
async def update_miner_profile_pinned_status(profile_cid: str, pinned: bool):
    ...
    try:
        async with aiosqlite.connect(DATABASE_NAME) as db:
            ...
            await db.commit()
            logging.info(
                f"Miner profile {profile_cid} pinned_locally status updated to {pinned}"
            )
    except aiosqlite.Error as e:
...
async def get_all_pinned_cids_from_db():
    ...
    try:
        async with aiosqlite.connect(DATABASE_NAME) as db:
            async with db.execute(
                "SELECT cid FROM pinned_cids WHERE status IN ('pinned', 'pending_pin')"
            ) as cursor:
...
async def get_cid_details(cid: str) -> tuple | None:
    ...
    try:
        async with aiosqlite.connect(DATABASE_NAME) as db:
            async with db.execute(
                "SELECT cid, status, retry_count FROM pinned_cids WHERE cid = ?", (cid,)
            ) as cursor:
                return await cursor.fetchone()
    except aiosqlite.Error as e:
